---
author: "Donggyun Kim"
title: "Post 01 - Reactivity in Shiny Apps"
date: "10/30/2017"
...
effort of rewriting code or changing initial value(_input_) to see different results    
(_output_). In other words, this trait makes it possible to compare each result of    
...
When you ![Run App](../images/image2.png){width=70} in RStudio, you will see how the app looks as the below image.  
  
![](../images/image3.png){width=550px}
...
* `titlePanel()` represents the title of the app. In this example,  
  ![](../images/image4.png){width=400px}  

* `slidebarLayout()` contains two functions, `sidebarPanel()` and `mainPanel()`.  
    * `sidebarPanel()` represents the initial value(_input_) of the app and takes the input   
from users. In this example,  
  ![](../images/image5.png){width=400px}  
    * `mainPanel()` represents the result(_output_) of the app. In this example,  
  ![](../images/image6.png){width=400px}  
\
...
Server function contains two _arguments_: `input` and `output`. This is because  
...
You may notice that I repeated two words, _input_ and _output_. You will see the  
reason why I did so.  
...
reactivity(input)
```
The _output_ of `reactivity` function depends on only _input_ unless the function is  
redefined. That is, _input_ **interacts** with _output_ through the `reactivity` function.  
```{r}
#set different input
input <- 3
reactivity(input)
...
![](../images/image7.png){width=400}  
the histogram shows the result with 6 bins.  
![](../images/image8.png){width=400}  
If the user sets input value as 15,  
![](../images/image9.png){width=400}  
the histogram shows the result with 15 bins.  
![](../images/image10.png){width=400}  

#5. Conclusion
**Reactivity** in Shiny apps happens in a condition where _input_ set by user  
**interacts** simultaneously with _output_. Since both _input_ and _output_ are  
